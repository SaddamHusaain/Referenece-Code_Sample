...
import CREATE_EVENT from "@test/models/.dist/graphql/mutations/createEvent.mutation";
import UPDATE_EVENT from "@test/models/.dist/graphql/mutations/updateEvent.mutation";
import PUBLISH_EVENT from "@test/models/.dist/graphql/mutations/publishEvent.mutation";
import LIST_EVENTS from "@test/models/.dist/graphql/queries/events.query";
import { BackstageState } from "../store";
import * as AppActions from "../actions/app.actions";
import * as EventActions from "../actions/event.actions";
import * as ErrorUtil from "@test/ui/build/utils/ErrorUtil";
...
import IEvent, { IEventGraphQL } from "@test/models/.dist/interfaces/IEvent";
...
import DELETE_EVENT from "@test/models/.dist/graphql/mutations/deleteEvent.mutation";
...
function* reCacheEventSaga(action: EventActions.ReCacheEventAction) {
  const { eventId, fromRemote }: { eventId: string; fromRemote?: boolean } =
    action.payload;

  try {
    let event: IEventGraphQL;
    const eventStates: EventReducerState = yield select(
    ...
    if (eventId === NEW_EVENT_ID) {
      ...
      const res = yield call(async () => {
        return await client.query({
          ...
          fetchPolicy: fromRemote ? "network-only" : undefined,
        });
      });
      event = eventStates.eventsCache[eventId];
...
function* saveEventSaga(action: EventActions.SaveEventAction) {
  ...
  // const hasChanged = yield ChangeUtil.hasEventChanged(event);
...
function* createEventRequestSaga(
  ...
  try {
    ...
    if (createEvent.seatingChartKey) {
...
function* publishEventRequestSaga({
  ...
  try {
    ...
    if (publishEvent.seatingChartKey && !isEdit) {
...
function* selectEventSeatingChartSaga(
  ...
  if (eventId.toLocaleLowerCase() !== NEW_EVENT_ID) {
    ...
    try {
      yield call(async () => {
        let seat = await seatsIOClient.events.retrieve(eventId);
        if (!event.seatingChartKey && !seat) {
          return await seatsIOClient.events.create(
            seatingChartKey,
            seatingId,
            true
          );
          ...
          return await seatsIOClient.events.update(seatingId, seatingChartKey);
        }
      });
    } catch (error: any) {
      console.error(error);
