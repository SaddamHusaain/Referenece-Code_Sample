...
import GET_ARTIST from "@test/models/.dist/graphql/queries/artist.query";
import LIST_ARTISTS from "@test/models/.dist/graphql/queries/artists.query";
import CREATE_ARTIST from "@test/models/.dist/graphql/mutations/createArtist.mutation";
import UPDATE_ARTIST from "@test/models/.dist/graphql/mutations/updateArtist.mutation";
...
function* navigateToPreviousStepWatch() {
  yield takeLatest(
    ArtistActionTypes.NAVIGATE_TO_PREVIOUS_STEP,
...
function* createHeadliningArtistWatch() {
  yield takeLatest(
    ArtistActionTypes.CREATE_HEADLINING_ARTIST,
    createHeadliningArtistSaga
  );
}
...
function* createHeadliningArtistSaga() {
  ...
  yield takeLatest(
    ...
    function* (action: ArtistActions.CreateArtistSuccessAction) {
      ...
      const performance: IPerformance = event
...
function* createOpeningArtistWatch() {
  yield takeLatest(
    ArtistActionTypes.CREATE_OPENING_ARTIST,
    createOpeningArtistSaga
  );
}
